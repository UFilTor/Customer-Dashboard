...
- A Vercel cron at `*/14 * * * *` (`/api/cron/warm`, gated by `CRON_SECRET`) refreshes the bulk routes proactively so users almost never see cold builds.

## Nothing is fetched server-side

`src/app/page.tsx` is a thin shell that fetches nothing, and it must stay that way. It used to server-render the (now-deleted) Status dashboard's payload; that was ~181 KB of the HTML response, about 85% of it, plus a ~4.5s cold block on every page load including Portfolio, which never read it. Every dashboard container owns its own client-side fetch. If you add a new dashboard, follow that: no SSR fetch, fetch on arrival.
