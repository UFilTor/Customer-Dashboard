...
// Retention reuses the onboarding history entry shape — the rendering brief
// is the same. Alias here so the rest of the file reads naturally.
type RetentionHistoryEntryT = OnboardingHistoryEntry;
...
export function RetentionContainer({ filter, filterLabel }: Props) {
  ...
  // Days currently in flight to /api/retention/day so the button can show a
  ...
  const dataRef = useRef<RetentionResponse | null>(null);
  ...
  const fetchData = useCallback(async (refresh = false) => {
    ...
    try {
      ...
      const url = `/api/retention${params.toString() ? `?${params.toString()}` : ""}`;
  ...
  const fetchDay = useCallback(async (dateKey: string) => {
    ...
    try {
      ...
      if (key !== "all") params.set("ownerIds", key);
      const res = await apiFetch(`/api/retention/day?${params.toString()}`);
      if (!res.ok) throw new Error(`Day unavailable (${res.status})`);
      const json: { meetings: RetentionMeetingEntry[] } = await res.json();
      setData((prev) => {
        if (!prev) return prev;
  ...
  // into the cached payload so RetentionBrief rerenders with the full
  ...
  useEffect(() => {
    ...
    (async () => {
      try {
        const res = await fetch(`/api/retention/history?dealIds=${historyDealIdsKey}`);
        if (!res.ok) return;
        const json: Record<string, RetentionHistoryEntryT[]> = await res.json();
        if (cancelled) return;
        setData((prev) => {
          if (!prev) return prev;
          const mergeFor = (dealId: string, current: RetentionHistoryEntryT[]) => {
